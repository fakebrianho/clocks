...
export const ring = (envMap, thickness, handleCheck) => {
	...
	let bRingMesh = new Mesh(ring, material)
	bRingMesh.position.set(0, 0, -0.25 * 0.5)
	...
	group.add(ringMesh, bRingMesh, outerRingMesh, innerRingMesh)
	return group
}
...
export const addLines = (texture) => {
	...
	for (let i = 0; i < 12; i++) {
		...
		l.position.set(
			Math.sin(0.523 * i) * 2.485,
			Math.cos(0.523 * i) * 2.485,
			0.125
		)
		group.add(l)
	}
